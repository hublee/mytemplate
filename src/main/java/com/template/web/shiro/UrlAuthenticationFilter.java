...
import com.template.common.utils.PatternUtils;


public class UrlAuthenticationFilter extends AuthorizationFilter {
	
	protected static final String[] blackUrlPathPattern = new String[] { "*.aspx*", "*.asp*",
		"*.php*", "*.exe*", "*.pl*", "*.py*", "*.groovy*", "*.sh*", "*.rb*", 
		"*.dll*", "*.bat*", "*.bin*", "*.dat*",  
	...
	protected boolean isAccessAllowed(ServletRequest request, ServletResponse response, Object mappedValue) {
    	if (!isLoginRequest(request, response)) {
            ...
            return userRealm.isPermitted(SecurityUtils.getSubject().getPrincipals(), StringUtils.replace(StringUtils.substring(exu_url,0,StringUtils.indexOf(exu_url, ".")), "/", ":"));
        }
        return true;
    }
